...
public class FunctionalProgTest {
	...
	private void biConsumer(FunctionalProgTest test) {
		...
		testBiConsumerLambda((k, v) -> {
			Comparator<People> keyComparator = Comparator.comparing(p -> p.name);
			Map<People, Integer> sortedMap = new TreeMap<>(keyComparator);
	...
	private void testConsumer() {
		...
		Consumer<String> c5 = (t) -> {int i = 10; if(i > 5) {
	...
	private <T,U> void testBiConsumerLambda(BiConsumer<T, U> biConsumer, Map<T,U> mapOfObjects) {
		Set<T> keyList = (Set<T>) mapOfObjects.keySet();
		U tempValue = null;
		for(T t : keyList) {
			tempValue = mapOfObjects.get(t);
			biConsumer.accept(t, tempValue);
		}
	}
}
