...
public class StreamTest {
	...
	public static void main(String[] args) {
		...
		Optional<Integer> optional = Stream.iterate(1, n -> n += 1).limit(5).max(c);
...
//		streamTest.testMutableReduce();
	...
	private void testSumOddNos() {
		System.out.println("Sum is : " + Stream.iterate(1, n-> n + 1)
		.filter(x -> x % 2 == 1)
		.limit(5)
	...
	private void testMapContents() {
		...
		employeeAttrs.values().parallelStream().filter(t -> true == t);
	}
	...
	private void testCombinedData() {
		...
		infinite.peek(System.out::print)
		.filter(x -> x % 2 == 1)
		.limit(5)
		.forEach(System.out::print);
		...
		.filter(x -> x % 2 == 1)
		.forEach(System.out::print);
		//11233455 - we are limiting numbers first ('limit'), so only first 5 numbers are  considered for further operation. Additionally,
